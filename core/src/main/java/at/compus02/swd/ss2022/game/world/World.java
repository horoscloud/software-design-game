...
public class World {
    ...
    private Array<TileFactory> worldObjects = new Array<>();
    ...
    public void create() {
        ...
        for (int i = -600; i < 600; i = i + 32) {
            ...
            tile.create("gras").setPosition(0,i);
            worldObjects.add(tile);

            for (int p = -600; p < 600; p = p + 32) {
                tile = new TileFactory();
                tile.create("gras").setPosition(p,i);
                worldObjects.add(tile);
            }
        }
    ...
    public Array<TileFactory> getWorldObjects() {
        return worldObjects;
    }
}
